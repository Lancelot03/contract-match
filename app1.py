...
with st.sidebar:
    ...
    # Changed label and help text for OpenAI
...
if analyze_btn:
    ...
    if not api_key:
        ...
        client = openai.OpenAI(api_key=api_key) # Changed Anthropic client to OpenAI client
        
        # Define System Prompt for Chat API
        system_prompt = "You are a legal contract analysis AI specializing in compliance review. Your task is to compare the two provided contract texts, identify violations, deviations, and risks, and output the analysis STRICTLY as a single JSON object conforming to the required schema and categories."
        ...
        try:
            with st.spinner("🤖 Analyzing contracts... This may take up to 30 seconds."):
                ...
                # Extract Response from OpenAI structure
                response_text = response.choices[0].message.content
                
                # Clean JSON (should be mostly clean due to response_format)
        ...
        except openai.APIError as e: # Changed error handling to openai.APIError
            st.error(f"OpenAI API Error: Check your API Key and usage limits. Details: {str(e)}")
